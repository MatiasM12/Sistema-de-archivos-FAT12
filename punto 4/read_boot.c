...
void file_info(Fat12Entry *entry) 
{
    ...
    switch(entry->attributes[0]){
        {
            case 0x10:  // Entrada tipo directorio
                printf("Directory: [%.7s]\n",  entry->filename); // Imprimo direcorio
                return;
...
int main() {
    ...
    for(i=0; i<4; i++) {  // Buscar particiones booteables      
    ...
    if(i == 4) {  // No se encontro nunguna particion booteable
