          ...
          int fileSize)
{
    ...
    fseek(in, firstCluster + ((fileFirstCluster - 2) * clusterSize), SEEK_SET); /* Nos posicionamos en el primer cluster del archivo */
...
void print_file_info(Fat12Entry *entry, unsigned short firstCluster, unsigned short clusterSize)
{
    ...
    switch (entry->attributes[0])
    {
        ...
        case 0x20:                                                                                 // Entrada tipo archivO
            printf("\nEl contenido del archivo %.7s%.7ses : ", entry->filename, entry->extension); // Imprimo archivo
...
int main()
{
    ...
    firstCluster = ftell(in) + (bs.root_dir_entries * sizeof(entry)); /* Obtenemos la posición del
                                                             primer byte del primer clúster de datos*/

    sizeOfCluster = bs.sector_size * bs.sector_cluster; // Tamaño en bytes de un clúster.
